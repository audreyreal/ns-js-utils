import { prettify, canonicalize, type NSScript } from "../../../nsdotjs";
 ...
 */
export async function handleLogin(context: NSScript, nationName: string, password: string): Promise<boolean> {
    ...
    if (match && canonicalize(match[0]) === canonNation) {
        ...
        context.statusBubble.success(`Logged in to nation: ${prettify(nationName)}`);
        return true;
    }
    context.statusBubble.warn(
